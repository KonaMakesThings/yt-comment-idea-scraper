...
IDEA_HEADERS = [
    "Review Status", "Idea Summary", "Original Comment", "Inferred Topic", "Source Video Title",
    "Comment Link", "Posted Time", "Creator Notes", "Duplicate Group", "Idea Type", "Video Link",
    "Username", "Comment Likes", "Classification Confidence", "Processed Time", "Comment ID",
]
...
class SheetStore:
    ...
    def ensure_layout(self) -> None:
        ...
        for title in (QUEUE_SHEET, "_Processed", "_RunLog"):
        ...
        if "_VideoBaseline" in existing:
            requests.append({"deleteSheet": {"sheetId": existing["_VideoBaseline"]["sheetId"]}})
        if requests:
    ...
    def _migrate_legacy_ideas(self) -> None:
        ...
        for source in legacy[1:]:
            ...
            by_name["Inferred Topic"] = by_name.get("Inferred Game/Topic", "")
    ...
    def _migrate_queue_headers(self) -> None:
        ...
        if current_headers[:len(IDEA_HEADERS)] == IDEA_HEADERS:
            return
        if "Comment ID" not in current_headers or "Idea Summary" not in current_headers:
            return
        positions = {name: index for index, name in enumerate(current_headers)}
        migrated = [IDEA_HEADERS]
        for source in queue[1:]:
            padded = source + [""] * len(current_headers)
            by_name = {name: padded[index] for name, index in positions.items()}
            by_name["Original Comment"] = by_name.get("Original Comment") or by_name.get("Raw Comment", "")
            by_name["Inferred Topic"] = by_name.get("Inferred Topic") or by_name.get("Inferred Game/Topic", "")
            by_name["Duplicate Group"] = by_name.get("Duplicate Group") or duplicate_group(
                by_name.get("Idea Summary", ""), by_name.get("Inferred Topic", ""),
            )
            migrated.append([by_name.get(header, "") for header in IDEA_HEADERS])
        self._update(f"'{QUEUE_SHEET}'!A1", migrated)
        if len(current_headers) > len(IDEA_HEADERS):
            self._clear(f"'{QUEUE_SHEET}'!Q:S")

    def _format_review_queue(self) -> None:
        sheet_id = self._sheet_id(QUEUE_SHEET)
        widths = [110, 360, 430, 190, 270, 150, 165, 250, 180]
        requests: list[dict[str, Any]] = [{
            ...
            "updateDimensionProperties": {
                "range": {"sheetId": sheet_id, "dimension": "COLUMNS",
                          "startIndex": 9, "endIndex": len(IDEA_HEADERS)},
    ...
    def _clear(self, range_: str) -> None:
        with_retry(lambda: self.service.spreadsheets().values().clear(
            spreadsheetId=self.id, range=range_, body={},
        ).execute())
    ...
    def ideas(self) -> dict[str, tuple[int, list[str]]]:
        ...
        rows = self._values(f"'{QUEUE_SHEET}'!A2:P")
    ...
    def write_result(self, comment: Comment, result: Classification, video: Video,
                     existing_idea: tuple[int, list[str]] | None,
        ...
        if result.is_idea:
            ...
            row = [
                old[IDEA_HEADERS.index("Review Status")] or "New", result.summary, comment.text,
                result.topic, video.title,
                f"https://www.youtube.com/watch?v={video.id}&lc={comment.id}",
                comment.published_at.isoformat(), old[IDEA_HEADERS.index("Creator Notes")],
                old[IDEA_HEADERS.index("Duplicate Group")] or duplicate_group(result.summary, result.topic),
                result.idea_type, f"https://www.youtube.com/watch?v={video.id}", comment.author_name,
    ...
    def _insert_queue_row(self, row: list[Any]) -> None:
        ...
        requests = [{
            ...
            "repeatCell": {
                "range": {"sheetId": sheet_id, "startRowIndex": 1,
                          "endRowIndex": 2, "startColumnIndex": 0, "endColumnIndex": 9},
        ...
        self._update(f"'{QUEUE_SHEET}'!A2:P", [row])
