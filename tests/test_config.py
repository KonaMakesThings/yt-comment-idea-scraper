...
def test_config_requires_all_secrets(monkeypatch):
    ...
    for name in ("GOOGLE_REFRESH_TOKEN", "SHEETS_REFRESH_TOKEN"):
        monkeypatch.delenv(name, raising=False)
...
def test_config_prefers_the_explicit_sheets_token(monkeypatch):
    for name in REQUIRED:
        monkeypatch.setenv(name, name.lower())
    monkeypatch.setenv("GOOGLE_REFRESH_TOKEN", "fallback-token")
    monkeypatch.setenv("SHEETS_REFRESH_TOKEN", "sheets-token")
    config = Config.from_env()
    assert config.sheets_refresh_token == "sheets-token"
