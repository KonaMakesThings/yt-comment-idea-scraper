...
class Classifier:
    ...
    def classify(self, comments, titles):
        ...
        return [Classification(c.id, True, "High", "recommendation", "Make it", "Example Topic", "Why") for c in comments]
...
def test_failed_batch_is_not_processed(capsys):
    class BrokenClassifier(Classifier):
        def classify(self, comments, titles):
            raise RuntimeError("private-resource-id")
    ...
    assert "private-resource-id" not in output
